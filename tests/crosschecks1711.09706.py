...
def getPhaseParameters(sp, ev, f_c = None):
    ...
    if isinstance(sp.m_chirp(), (np.ndarray, Sequence)):
        m_chirp = sp.m_chirp()[0]
    else:
        m_chirp = sp.m_chirp()

    omega_gw = interp1d(t, 2*omega_s, kind='cubic', bounds_error=False, fill_value=(0.,0.) )
    Phit = np.cumsum([quad(lambda t: omega_gw(t), t[i-1], t[i], limit=200, epsrel=1e-13, epsabs=1e-13)[0] if not i == 0 else 0. for i in range(len(t)) ])
    #Phit = solve_ivp(lambda t,y: omega_gw(t), [t[0], t[-1]], [0.], t_eval=t, atol=1e-13, rtol=1e-13).y[0]
    Phi = Phit #- Phit[-1]

    # Use t_c = t[-1] as reference point
...
def plotPhase(sp, ev_acc, ev_nacc, f_c=None):
    ...
    #PhiTild0 = - 3./4.*(8.*np.pi*sp.m_chirp()*f_gw)**(-5./3.) + 3./4.*(8.*np.pi*sp.m_chirp()*f_c)**(-5./3.)
    ...
    plt.plot(f_gw*ms.year_to_pc*3.17e-8, np.abs(DeltaPhi), label=r'$\Delta\tilde{\Phi}^{code}$')

    mu_interp = interp1d(f_gw, mu(sp, f_gw, f_gw[0]), kind='cubic', bounds_error=False, fill_value='extrapolate')
    #Phi_ana = np.cumsum([quad(lambda f: f**(-8./3.)/L(sp, f)/mu_interp(f), f_gw[i-1], f_gw[i], limit=200, epsrel=1e-13, epsabs=1e-13)[0] if not i == 0 else 0. for i in range(len(f_gw)) ][::-1])[::-1]
    ...
    #tpt_ana = np.cumsum([quad(lambda f: f**(-11./3.)/L(sp, f)/mu_interp(f), f_gw[i-1], f_gw[i], limit=200, epsrel=1e-13, epsabs=1e-13)[0] if not i==0 else 0. for i in range(len(f_gw)) ][::-1])[::-1]
    tpt_ana = solve_ivp(lambda f,y: -f**(-11./3.)/L(sp, f)/mu_interp(f), [f_gw[-1], f_gw[0]], [0.], t_eval=f_gw[::-1], atol=1e-13, rtol=1e-13).y[0][::-1]
    print("tpt_ana=", tpt_ana)
    ...
    plt.plot(f_gw*ms.year_to_pc*3.17e-8, np.abs(DeltaPhi_ana), label=r'$\Delta\tilde{\Phi}^{paper}$')
    plt.plot(f_gw*ms.year_to_pc*3.17e-8, np.abs(DeltaPhi - DeltaPhi_ana), label=r'$\Delta \Delta\tilde{\Phi}$')

    # Code no accretion
    f_gw_nacc, _, __, PhiTild_nacc = getPhaseParameters(sp, ev_nacc, f_c=f_c)
    f_c = f_gw_nacc[-1]
    PhiTild0 =  (8.*np.pi*sp.m_chirp())**(-5./3.) * (-3./4. * f_gw_nacc**(-5./3.) - 5./4. * f_gw_nacc * f_c**(-8./3.) + 2.*f_c**(-5./3.))
    #PhiTild0 = - 3./4.*(8.*np.pi*sp.m_chirp()*f_gw_nacc)**(-5./3.) + 3./4.*(8.*np.pi*sp.m_chirp()*f_c)**(-5./3.)
    ...
    plt.plot(f_gw*ms.year_to_pc*3.17e-8, np.abs(deltaPhi/DeltaPhi), label=r'$\delta\tilde{\Phi}^{code}/\Delta\tilde{\Phi}$')

    # Paper no accretion
    #Phi_ana = np.cumsum([quad(lambda f: f**(-8./3.)/L(sp, f, accretion=False), f_gw_nacc[i-1], f_gw_nacc[i], limit=200, epsrel=1e-13, epsabs=1e-13)[0] if not i == 0 else 0. for i in range(len(f_gw_nacc)) ][::-1])[::-1]
    Phi_ana = solve_ivp(lambda f,y: -f**(-8./3.)/L(sp, f, accretion=False), [f_gw_nacc[-1], f_gw_nacc[0]], [0.], t_eval=f_gw_nacc[::-1], atol=1e-13, rtol=1e-13, method='LSODA').y[0][::-1]
    Phi_ana = 10./3. * (8.*np.pi*sp.m_chirp())**(-5./3.) * (Phi_ana - Phi_ana[-1])
    #tpt_ana = np.cumsum([quad(lambda f: f**(-11./3.)/L(sp, f, accretion=False), f_gw_nacc[i-1], f_gw_nacc[i], limit=200, epsrel=1e-13, epsabs=1e-13)[0] if not i==0 else 0. for i in range(len(f_gw_nacc)) ][::-1])[::-1]
    tpt_ana = solve_ivp(lambda f,y: -f**(-11./3.)/L(sp, f, accretion=False), [f_gw_nacc[-1], f_gw_nacc[0]], [0.], t_eval=f_gw_nacc[::-1], atol=1e-13, rtol=1e-13, method='LSODA').y[0][::-1]
    ...
    plt.plot(f_gw*ms.year_to_pc*3.17e-8, np.abs(deltaPhi_ana/DeltaPhi_ana), label=r'$\delta\tilde{\Phi}^{paper}/\Delta\tilde{\Phi}$')
...
R0 = 100.*sp_1.r_isco()
ev_nacc = inspiral.Classic.Evolve(sp_1, R0, a_fin=sp_1.r_isco(), opt=inspiral.Classic.EvolutionOptions(accuracy=1e-13, accretion=False))
ev_acc = inspiral.Classic.Evolve(sp_1, R0, a_fin=sp_1.r_isco(), opt=inspiral.Classic.EvolutionOptions(accuracy=1e-13, accretion=True, verbose=1))
