...
def h_n_orbit(n, hs, ko):
    ...
    TODO:
        Check redshift, luminosity distance inclusion
    """
    inclination_angle, beta = get_observer_orbit_angles(hs, ko)
    s_i = np.sin(inclination_angle); c_i = np.cos(inclination_angle)
    ...
    e = np.clip(ko.e, 1e-10, 1.)
    return C_n_plus(n, beta, e), S_n_plus(n, beta, e), C_n_cross(n, beta, e), S_n_cross(n, beta, e)

def get_observer_orbit_angles(hs, ev, t_grid=None):
    """
    ...
    Parameters:
        ...
        t_grid (array_like) : The times at which to evaluate the angles, will call the _int interpolation objects in ev
                                If not provided, the ev values are taken by themselves (could be a KeplerOrbit object as well)
    ...
    """
    if t_grid is None:
        omega = ev.periapse_angle
        Omega = ev.longitude_an
        iota = ev.inclination_angle
    else:
...
def h(hs, ev, t_grid, phi_0=0., acc=1e-13):
    ...
    sol = solve_ivp(phi_dot, [t_grid[0], t_grid[-1]], [phi_0], t_eval=t_grid, rtol=acc, atol=acc, max_step=t_grid[-1]-t_grid[0])  # calculate the orbital phase at the given time steps t_grid
    phi = sol.y[0]
    e = ev.e_int(t_grid)
